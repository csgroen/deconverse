---
title: "`deconverse` introduction: full explained pipeline with PBMCs"
...
Now, we're ready to build the reference as an `hscrefehttps://xfer.curie.fr/get/nil/oD0PXAACeu0/scref_cache.ziprence` object. We use `deconverse::new_hscreference` and give our Seurat object `pbmc_train`, the l1 and l2 annotation variables in order, a project name, and a batch_id variable which could correspond to patient in another context, but here is just the same value for all cells.
...
pbmc_bench <- pbmc_bench |> 
    ...
    deconvolute_all(pbmc_ref, methods = c("dwls", "svr", "ols",
                                          "autogenes", "bisque"))
```
...
```{r}
samp_annot$data_processing.2[1] #Gencode 26, ~ Ensembl 91
library(biomaRt)
